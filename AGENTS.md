...
- `.env` is not read by the app anymore (previously `routers/auth.py` via `load_dotenv()`). `database.py` hardcodes `sqlite:///./reader.db` and ignores `DATABASE_URL`.
...
- `database.py` has no `User` model and no `user_id` columns. If upgrading an old DB, run `python migrate_db.py` (it drops `user_id` columns from the data tables and the `users` table).

## Database

- SQLite `reader.db` is git-ignored (`*.db`) and committed nowhere; it is real local dev data (54MB). Never delete it casually.
- Tables are created at startup via `Base.metadata.create_all` in `main.py`. Alembic is in requirements but unused; `migrate_db.py` does manual raw-SQLite `ALTER TABLE` to add columns (e.g. `is_cached`, `cached_at`). For schema changes follow that pattern: add column to model + idempotent ALTER in `migrate_db.py`.
- Chapter content is lazy: `Chapter.content` stays NULL, fetched from the source site on demand and cached only if < 50KB (`routers/books.py:190`).

## Book sources / parsers

- `parsers/parser_loader.py` discovers parsers two ways: (1) subclasses of `BaseBookSourceParser` in `sources/*.py`, (2) entries in `sources/sources.json` (git-ignored user data).
- Adding a source = either drop a `sources/xx_parser.py` subclass or append to `sources.json`. Parser API: `search_books`, `get_book_info`, `get_chapter_list`, `update_chapter_list`, `get_chapter_content`, `can_handle_url`, `get_parser_name`. Parsing is CSS-selector config driven (`cfg_template` in `parsers/base_parser.py`); override methods per-site.
...
- NOT pytest and no CI. `tests/` are standalone async scripts that hit live websites and the real `reader.db`: run `python tests/test_parsers.py` (edit `__main__` at the bottom to select which source), `python tests/test_chapter.py`.
